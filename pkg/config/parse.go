...
func ParseClientConfig(filePath string, remote_Port int) (
	...
	if filePath == "" {
		content, err = GetRenderedConfFromDefaultConf(remote_Port)
	} else if strings.HasPrefix(filePath, "http://") || strings.HasPrefix(filePath, "https://") {
		content, err = GetRenderedConfFromUrl(filePath)
	} else {
